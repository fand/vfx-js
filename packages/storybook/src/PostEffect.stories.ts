import type { VFXProps, VFXPostEffect, VFXPass } from "@vfx-js/core";
...
// ─────────────────────────────────────────────────────────────────────────────
// Stable Fluid — full Navier-Stokes pipeline
// Passes: curl → vorticity → divergence → 20x pressure → grad-sub → advect vel
//         → advect dye → display
// ─────────────────────────────────────────────────────────────────────────────
...
// Pressure init: output 0 (start Jacobi from zero each frame)
...
// Jacobi pressure iteration: reads src (prev pressure) + divergence named buffer
...
// Gradient shader reads from whichever pressure buffer holds the final result.
...
// Build the 20-pass Jacobi pressure solver chain.
// Alternates writes between "p_a" and "p_b" using src (inputTexture) chain.
...
export const StableFluid = {
  ...
  render: (args: FluidArgs) => {
    ...
    const vfx = initVFX({
      postEffect: [
        // Save original canvas to "canvas" named buffer
...
// Stable fluid simulation using multipass shaders.
// Velocity is stored in backbuffer RG (encoded to [0,1]),
// dye color in BA channels.
...
const float VS = ${VELOCITY_SCALE.toFixed(1)};
const float DT = 0.016;
const float CURL = 30.0;
const float VELOCITY_DISSIPATION = 0.2;
const float SPLAT_RADIUS = 0.0025;
const float SPLAT_FORCE = 50.0;

vec2 decVel(vec2 e) { return (e - 0.5) * 2.0 * VS; }
vec2 encVel(vec2 v) { return clamp(v / (2.0 * VS) + 0.5, 0.0, 1.0); }

float getCurl(vec2 p, vec2 tx) {
    float L = decVel(texture(backbuffer, p - vec2(tx.x, 0)).rg).y;
    float R = decVel(texture(backbuffer, p + vec2(tx.x, 0)).rg).y;
    float T = decVel(texture(backbuffer, p + vec2(0, tx.y)).rg).x;
    float B = decVel(texture(backbuffer, p - vec2(0, tx.y)).rg).x;
    return 0.5 * (R - L - T + B);
}
...
void main() {
    ...
    // Vorticity confinement
    float cC = getCurl(uv, tx);
    float cL = getCurl(uv - vec2(tx.x, 0), tx);
    float cR = getCurl(uv + vec2(tx.x, 0), tx);
    float cT = getCurl(uv + vec2(0, tx.y), tx);
    float cB = getCurl(uv - vec2(0, tx.y), tx);
    vec2 force = 0.5 * vec2(abs(cT) - abs(cB), abs(cR) - abs(cL));
    float fLen = length(force);
    force = fLen > 0.0001 ? force / fLen : vec2(0);
    force *= CURL * cC;
    force.y *= -1.0;

    // Semi-Lagrangian self-advection
    vec2 vel = decVel(texture(backbuffer, uv).rg);
    vec2 coord = uv - DT * vel * tx;
    vec2 advVel = decVel(texture(backbuffer, coord).rg);
    advVel += force * DT;

    // Gaussian splat from mouse velocity
    vec2 mouseUV = mouse / resolution;
    vec2 p = uv - mouseUV;
    p.x *= resolution.x / resolution.y;
    vec2 splatVel = (mouseDelta / resolution) * SPLAT_FORCE;
    advVel += exp(-dot(p, p) / SPLAT_RADIUS) * splatVel;

    // Dissipation
    advVel /= 1.0 + VELOCITY_DISSIPATION * DT;
    advVel = clamp(advVel, -VS, VS);

    // Pass through dye in BA
    vec4 prev = texture(backbuffer, uv);
    outColor = vec4(encVel(advVel), prev.ba);
}
`;
...
const float VS = ${VELOCITY_SCALE.toFixed(1)};
const float DT = 0.016;
const float DENSITY_DISSIPATION = 0.5;
const float SPLAT_RADIUS = 0.0025;
...
void main() {
    ...
    // Advect dye using velocity
    vec4 velData = texture(velocity, uv);
    vec2 vel = decVel(velData.rg);
    vec2 coord = uv - DT * vel * tx;
    vec4 prev = texture(backbuffer, coord);
    vec2 dye = prev.ba;

    // Dye dissipation
    dye /= 1.0 + DENSITY_DISSIPATION * DT;

    // Gaussian dye splat at mouse position
    vec2 mouseUV = mouse / resolution;
    vec2 p = uv - mouseUV;
    p.x *= resolution.x / resolution.y;
    float splat = exp(-dot(p, p) / SPLAT_RADIUS);
    float hasMouse = step(0.001, length(mouseDelta));
    vec3 dyeColor = 0.5 + 0.5 * cos(time * 0.5 + vec3(0, 2.094, 4.189));
    dye += dyeColor.rg * splat * hasMouse * 0.5;

    // Display: dye color from BA, derive blue
    vec3 color = vec3(dye.x, dye.y, (dye.x + dye.y) * 0.5);

    // Encode velocity (pass through from velocity pass) + dye
    outColor = vec4(velData.rg, clamp(dye, 0.0, 1.0));
}
`;

export const StableFluid = {
    render: () => {
        const timer = new Timer(0, [0, 600]);
        timer.togglePlay();
        document.body.append(timer.element);

        // Track mouse delta (physical pixels)
        ...
        window.addEventListener("mousemove", (e) => {
            ...
            const ny = (window.innerHeight - e.clientY) * pr;
            dX += nx - mX;
            dY += ny - mY;
            mX = nx;
            mY = ny;
            ...
            shader: [
                {
                    ...
                    uniforms: {
                        mouseDelta: () => {
                            ...
                            dX = 0;
                            dY = 0;
                            return [lastDX, lastDY] as [number, number];
                        },
                    },
                },
                {
                    ...
                    uniforms: {
                        mouseDelta: () =>
                            [lastDX, lastDY] as [number, number],
                    },
                },
            ] as VFXPass[],
            backbuffer: true,
