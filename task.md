...
- [x] **1-1: Add Effect types to `packages/vfx-js/src/types.ts`**
...
- [x] **1-2: Export new types from `packages/vfx-js/src/index.ts`**

## Phase 2: gl/* extensions

- [x] **2-1: Extend `gl/texture.ts` — configurable filter**
...
- [x] **2-2: Extend `gl/framebuffer.ts` — configurable wrap/filter**
...
- [x] **3-1: `packages/vfx-js/src/effect-geometry.ts` (new)**
...
- [x] **3-2: `packages/vfx-js/src/effect-host.ts` (new)**
...
- [x] **3-3: `packages/vfx-js/src/effect-chain.ts` (new)**
...
- [x] **4-1: `vfx-player.ts` — `addElement` Effect branch**
...
- [x] **4-2: `vfx-player.ts` — `render()` Effect branch**
...
- [x] **4-3: `vfx-player.ts` — `removeElement` Effect branch**
  - Call `chain.dispose()` only (it handles effect + host + intermediate RT cleanup)

- [x] **4-4: `vfx-player.ts` — `VFXPostEffect.effect`**
...
- [x] **4-5: `vfx.ts` — pass `effect` through**
  - `add()` / `addHTML()` forward `effect` to the player (mostly pass-through)

## Phase 5: Tests

- [x] **5-1: `packages/vfx-js/src/effect-host.test.ts`**
...
- [x] **5-2: `packages/vfx-js/src/effect-chain.test.ts`**
...
- [x] **6-1: `packages/storybook/src/Effect.stories.ts`**
...
- [x] **7-1: Build check** — `npm --workspace=@vfx-js/core run build` produces dual ESM/CJS; new types appear in `lib/esm/index.d.ts`
- [x] **7-2: Zero-runtime-dep check** — grep `packages/storybook` build output; no runtime imports of `@vfx-js/core`
- [x] **7-3: Existing tests/lint** — `npm test && npm run lint` pass

---

## Phase 8: Pad model refactor

> **NOTE (2026-04-26)**: Phase 8 tasks all landed (per `progress.md`
> 8-1〜8-9), but the pad-model itself was subsequently **replaced by
> the rect-model** — see `plan-rect.md`. Concepts referenced below
> (`padAdd`, `srcInnerRect`, `uvInnerRect`, `uvInnerDst`,
> `fullscreenPad`, `outputSize`) **no longer exist** in the codebase.
> The rect-model uses `Effect.outputRect(dims) → ElementRect`,
> `rectContent` / `rectSrc` uniforms, and `uvContent` / `uvSrc`
> varyings. Tasks marked `[x]` reflect their landing state at commit
> time, not the current code shape.
...
- [x] **8-1**: Update `Effect.outputSize` signature in `packages/vfx-js/src/types.ts`
...
- [x] **8-2**: Rewrite chain pad propagation in `packages/vfx-js/src/effect-chain.ts`
...
- [x] **8-3**: Update `packages/vfx-js/src/effect-host.ts`
...
- [x] **8-4**: Update `#renderEffectElement` and `#addEffectElement` in `packages/vfx-js/src/vfx-player.ts`
...
- [x] **8-5**: Update `packages/storybook/src/effects/bloom.ts`
...
- [x] **8-6**: Update `packages/storybook/src/effects/posterize.ts` *(file later deleted as unused — `21a7134`)*
...
- [x] **8-7**: Update `packages/storybook/src/Effect.stories.ts`
...
- [x] **8-8**: Update `packages/vfx-js/src/effect-chain.test.ts` and `effect-host.test.ts`
...
- [x] **8-9**: Update `progress.md` as Phase 8 tasks land
